...
# 0. 페이지 기본 설정
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="줏대있는 개미의 철벽 스크리너")
...
def normalize_date(raw: str) -> str:
    """날짜 문자열을 YYYY.MM.DD 포맷으로 통일 (대시/점/슬래시/ISO 모두 처리)"""
    if not raw: return ""
...
def get_macro_indicators():
    ...
    def fetch_last_two_close(symbol):
        ...
        if pd.isna(close_prev) or close_prev == 0: chg_pct = 0.0
        else: chg_pct = ((close_curr - close_prev) / close_prev) * 100
        return close_curr, chg_pct

    for key, symbol in [("kospi", "KS11"), ("sp500", "^GSPC"), ("usd_krw", "USD/KRW"), ("us10y", "^TNX")]:
        try: result[key] = fetch_last_two_close(symbol)
...
# ─────────────────────────────────────────────
# 2. 하이브리드 3단 엔진 (네이버 기본 + 다음 수급 + PC 우회)
# ─────────────────────────────────────────────
...
def get_naver_stock_data(code, run_mode):
    ...
    warnings_list = []
    stock_name, current_price, per, pbr, industry_per = "데이터 로딩중", 0.0, 0.0, 0.0, 0.0
    net_foreigner, net_institution, target_date_str = 0.0, 0.0, ""
    session = requests.Session()

    # ══ 엔진 1: 네이버 모바일 API → 기본정보 ══
    try:
        n_headers = {**base_headers, 'Referer': f'https://m.stock.naver.com/domestic/stock/{code}/total'}
        res = session.get(f"https://m.stock.naver.com/api/stock/{code}/basic", headers=n_headers, timeout=5)
        if res.status_code == 200:
            data = res.json().get('result', {})
            name  = (data.get('stockName') or data.get('itemName') or '').strip()
            price = safe_float(data.get('closePrice') or data.get('ncv') or '0')
        ...
        warnings_list.append(f"기본정보 지연")

    # ══ 엔진 2: 다음 금융 API → 메이저 수급 전용 (보안 헤더 강화) ══
    try:
        d_headers = {
            **base_headers, 
        ...
        res_d = session.get(f"https://finance.daum.net/api/investor/days?page=1&perPage=5&symbolCode=A{code}", headers=d_headers, timeout=6)
        if res_d.status_code == 200:
            daum_items = res_d.json().get('data', [])
            if daum_items:
                t_idx = 1 if run_mode == "장중 (전일 확정 데이터 조회)" else 0
                if t_idx < len(daum_items):
                    t = daum_items[t_idx]
                    target_date_str = normalize_date(str(t.get('date', '')))
                    net_frgn_str = get_any_key(t, ['foreignStraightPurchaseVolume', 'foreignStraightPurchaseVol', 'foreignNetBuyVolume', 'foreignNetBuyVol'])
                    net_inst_str = get_any_key(t, ['institutionStraightPurchaseVolume', 'institutionStraightPurchaseVol', 'institutionNetBuyVolume', 'institutionNetBuyVol'])
                    net_foreigner   = safe_float(net_frgn_str)
                    net_institution = safe_float(net_inst_str)
        else:
            warnings_list.append(f"수급망 응답 오류")
    except Exception as e:
        warnings_list.append(f"수급망 지연")

    # ══ 엔진 3: 네이버 PC HTML → 유실 항목 보완 (최후 보루) ══
    needs_basic   = (current_price == 0.0 or stock_name == "데이터 로딩중")
    needs_investor = (not target_date_str or (net_foreigner == 0.0 and net_institution == 0.0))

    if needs_basic or needs_investor:
        try:
            n_headers = {**base_headers, 'Referer': 'https://finance.naver.com/'}
            if needs_basic:
                res_m = session.get(f"https://finance.naver.com/item/main.naver?code={code}", headers=n_headers, timeout=7)
                soup = BeautifulSoup(res_m.content.decode('euc-kr', 'replace'), 'html.parser')

                if stock_name == "데이터 로딩중":
                    wrap = soup.find("div", {"class": "wrap_company"})
                    if wrap and wrap.find("h2"): stock_name = wrap.find("h2").text.strip()
                if current_price == 0.0:
                    no_today = soup.select_one("div.rate_info p.no_today span.blind")
                    if no_today: current_price = safe_float(no_today.text)
            ...
            if needs_investor:
                res_f = session.get(f"https://finance.naver.com/item/frgn.naver?code={code}", headers=n_headers, timeout=7)
                sub_soup = BeautifulSoup(res_f.content.decode('euc-kr', 'replace'), 'html.parser')
                ...
                if frgn_table:
                    ...
                    for row in rows:
                        ...
                        if len(tds) >= 7:
                            if v_count == t_idx:
                                if not target_date_str: target_date_str = normalize_date(tds[0].text.strip())
                                if net_institution == 0.0: net_institution = safe_float(tds[5].text)
                                if net_foreigner == 0.0: net_foreigner = safe_float(tds[6].text)
                                break
                            v_count += 1
        except Exception as e:
            warnings_list.append(f"PC 우회 지연")

    if warnings_list: st.warning("⚠️ 데이터 동기화 알림: " + " | ".join(warnings_list[:2]))
...
def get_dart_mapping(api_key):
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(res.content)) as z: xml_data = z.read('CORPCODE.xml')
        root = ET.fromstring(xml_data)
        return {n.find('stock_code').text: n.find('corp_code').text for n in root.findall('list') if n.find('stock_code').text}
    except Exception as e:
...
def get_dart_fundamentals(api_key, stock_code):
    ...
    for y in [str(datetime.now().year - 1), str(datetime.now().year - 2)]:
        for acnt in ["fnlttCnsldtAcnt", "fnlttSinglAcnt"]:
            url = f"https://opendart.fss.or.kr/api/{acnt}.json?crtfc_key={api_key}&corp_code={corp_code}&bsns_year={y}&reprt_code=11011"
            try:
                ...
                def get_val(keys):
                    for item in data:
                        nm = item['account_nm'].replace(" ", "")
                        if any(nm == kw or nm == f"{kw}(손실)" or nm == f"연결{kw}" or nm == f"연결{kw}(손실)" for kw in keys):
                            return safe_float(item['thstrm_amount'])
                    return 0.0

                rev  = get_val(['매출액', '영업수익'])
                op   = get_val(['영업이익'])
                eq   = get_val(['자본총계'])
                liab = get_val(['부채총계'])
                net  = get_val(['당기순이익', '분기순이익', '반기순이익'])

                return {
                    "year":       y,
                    "acnt_type":  "연결" if acnt == "fnlttCnsldtAcnt" else "별도",
                    "op_margin":  (op   / rev  * 100) if rev  > 0 else 0.0,
                    "debt_ratio": (liab / eq   * 100) if eq   > 0 else 0.0,
                    "roe":        (net  / eq   * 100) if eq   > 0 else 0.0,
                }
            except Exception:
                continue
    return None
...
if basic_data is None or basic_data['price'] == 0.0:
    st.error("데이터 서버 통신 실패. 네트워크 상태를 재확인해 주세요.")
    st.stop()

if dart_data:
    st.info(f"✅ DART 재무 엑스레이 연결 성공 | {dart_data['year']}년 {dart_data['acnt_type']} 재무제표 기준")
...
is_real_etf = (now_per == 0.0 and now_pbr == 0.0) and (
    any(kw in basic_data['name'].upper() for kw in
        ["KODEX","TIGER","KBSTAR","ARIRANG","KOSEF","HANARO","SOL","ACE","TIMEFOLIO","FOCUS","ETF","스팩"])
...
if is_real_etf:
    ...
    elif ind_per > 0 and now_per <= ind_per * 0.85: f1_score, f1_desc = 90, f"PER {now_per}배 (업종 대비 저평가)"
...
if not is_chart_available or pd.isna(ema50) or pd.isna(ema20):
    f3_score, f3_desc = 50, "이평선 유보"
    f4_score, f4_desc = 50, "거래량 유보"
    f9_score, f9_desc = 50, "지지선 유보"
else:
    ...
    if latest['Volume'] == 0: f4_score, f4_desc = 0, "🚨 거래정지 또는 비정상 호가 감지"
    else:
        ...
        if vol_ratio >= 2.0:   f4_score, f4_desc = (95, f"🔥 20일평균 {vol_ratio:.1f}배 돌파 양봉") if is_bullish else (25, f"🚨 20일평균 {vol_ratio:.1f}배 폭발 장대음봉")
    ...
    if pd.isna(ema20) or ema20 <= 0 or pd.isna(ema20_prev):
        f9_score, f9_desc = 50, "EMA20 데이터 부족으로 지지선 유보"
    else:
...
# ─────────────────────────────────────────────
if is_real_etf:
    total_score = int((f3_score*1.5 + f4_score*1.5 + f5_score*0.5 + f6_score*1.5 + f7_score*0.5 + f8_score*0.5 + f9_score*1.0) / 7.0)
else:
    total_score = int((f1_score*1.0 + f2_score*1.0 + f3_score*1.5 + f4_score*1.5 + f5_score*0.5 + f6_score*1.5 + f7_score*0.5 + f8_score*0.5 + f9_score*1.0) / 9.0)
