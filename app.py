...
# ─────────────────────────────────────────────
# 2. 네이버 증권 스크래핑 (차단 방어용 Session 처리)
# ─────────────────────────────────────────────
...
def get_naver_stock_data(code, run_mode):
    ...
    # [수정①] warnings_list를 실제로 화면에 표시하도록 수정
    warnings_list = []
    ...
    # [수정①] 파싱 경고 실제 표시
    if warnings_list:
        st.warning("네이버 파싱 경고: " + " | ".join(warnings_list))
...
def get_dart_mapping(api_key):
    if api_key == "여기에_선생님_키를_붙여넣으세요" or not api_key:
        return {}
    try:
        ...
        return {
            node.find('stock_code').text: node.find('corp_code').text
            for node in root.findall('list')
            if node.find('stock_code').text
        }
    except Exception as e:
        # [수정⑥] DART 매핑 실패 시 사용자에게 명확히 알림
    ...
    for year_offset in [1, 2]:
        ...
        for acnt_type in ["fnlttCnsldtAcnt", "fnlttSinglAcnt"]:
            ...
            try:
                ...
                def get_val(acct_names):
                    for item in data:
                        if item['account_nm'] in acct_names:
...
# 4. 앱 본문 시작
# ─────────────────────────────────────────────
st.title("📱 주식저장소 개미의 무결점 퀀트 스크리너")

macro_data = get_macro_indicators()
if macro_data:
    m_cols = st.columns(4)
    with m_cols[0]: st.metric("국내 코스피 지수", f"{macro_data['kospi'][0]:,.2f}", f"{macro_data['kospi'][1]:+.2f}%")
    with m_cols[1]: st.metric("미국 S&P 500 지수", f"{macro_data['sp500'][0]:,.2f}", f"{macro_data['sp500'][1]:+.2f}%")
...
if is_real_etf:
    ...
    # [수정④] else 분기에서 f8_score/f8_desc 명시적 초기화 (슬라이더 전에 기본값 확보)
    f8_score, f8_desc = 50, "정성적 판단 (가중치↓)"

# ── F3·F4·F9: 차트 기반 ──────────────────────
if not is_chart_available or pd.isna(ema50) or pd.isna(ema20):
    f3_score, f3_desc = 50, "상장 초기/서버 점검으로 이평선 유보"
    f4_score, f4_desc = 50, "상장 초기/서버 점검으로 거래량 유보"
    f9_score, f9_desc = 50, "상장 초기/서버 점검으로 지지선 유보"
else:
    rsi_display = f"{rsi_val:.1f}" if not pd.isna(rsi_val) else "N/A"
    f3_score = 95.0 if now_price > ema20 > ema50 else 35.0
    f3_desc = (f"상승 정배열 안착 (RSI: {rsi_display})"
                if f3_score == 95.0
                else f"역배열 또는 추세 이탈 (RSI: {rsi_display})")
    ...
    if latest['Volume'] == 0:
        ...
        if vol_ratio >= 2.0:
            f4_score, f4_desc = (
                (95, f"🔥 20일평균 {vol_ratio:.1f}배 돌파 양봉")
                if is_bullish else
                (25, f"🚨 20일평균 {vol_ratio:.1f}배 폭발 장대음봉")
            )
        elif vol_ratio >= 1.0:
            f4_score, f4_desc = (75 if is_bullish else 65), f"20일평균 {vol_ratio:.1f}배 (평이한 매매)"
        else:
            f4_score, f4_desc = 40, f"20일평균 {vol_ratio:.1f}배 소외"
    ...
    if pd.isna(ema20) or ema20 <= 0 or pd.isna(ema20_prev):
        ...
        if disparity < 50 or disparity > 200:
            f9_score, f9_desc = 50, "⚠️ 가격 왜곡 감지 (분할/배당락 의심)"
        elif 98 <= disparity <= 102:
            if is_ema20_rising:
                f9_score, f9_desc = 95, f"🎯 핵심 지지선 안착 (추세 우상향, 이격도 {disparity:.1f}%)"
            else:
                f9_score, f9_desc = 60, f"⚠️ 지지선 부근이나 하락 추세 (이격도 {disparity:.1f}%)"
        elif disparity > 102:
            f9_score, f9_desc = int(max(30.0, 55.0 - (disparity - 102) * 1.5)), f"EMA20 상단 이탈 — 단기 과매수 (이격도 {disparity:.1f}%)"
        else:
            f9_score, f9_desc = int(max(30.0, 55.0 - (98 - disparity) * 1.5)), f"EMA20 하단 이탈 — 지지선 이탈 (이격도 {disparity:.1f}%)"
...
# [수정②] ETF이면 슬라이더 불필요 — is_real_etf 조건 추가
...
if is_real_etf:
    total_score = int(
        (f3_score*1.5 + f4_score*1.5 + f5_score*0.5 + f6_score*1.5 + f7_score*0.5 + f8_score*0.5 + f9_score*1.0)
        / 7.0
    )
else:
    total_score = int(
        (f1_score*1.0 + f2_score*1.0 + f3_score*1.5 + f4_score*1.5 + f5_score*0.5 + f6_score*1.5 + f7_score*0.5 + f8_score*0.5 + f9_score*1.0)
        / 9.0
    )
